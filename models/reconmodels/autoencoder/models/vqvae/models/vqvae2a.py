...
class coder(nn.Module):
    def __init__(self,in_channels:int,hidden_channels:int):
        super().__init__()
        assert log2(in_channels/hidden_channels) % 1 == 0, 'Downscale must be a power of 2'
        downscale_steps = int(log2(in_channels/hidden_channels))
        print('downscale_steps:',downscale_steps)
        layers = []
        c_channel, n_channel = in_channels, in_channels // 2
        for _ in range(downscale_steps):
            ...
            layers.append(nn.Conv2d(n_channel, n_channel, 3, stride=1, padding=1))
            c_channel, n_channel = n_channel, n_channel//2
        layers.append(nn.Conv2d(c_channel, c_channel, 3, stride=1, padding=1))
        for _ in range(downscale_steps):
            layers.append(nn.Conv2d(c_channel, c_channel, 3, stride=1, padding=1))
        layers.append(nn.BatchNorm2d(c_channel))
...
class CodeLayer(nn.Module):
    def __init__(self, in_channels: int, embed_dim: int, nb_entries: int):
        ...
        self.conv_in = coder(in_channels,embed_dim)
        self.dim = embed_dim
...
class VQVAE2(pl.LightningModule):
    def __init__(self,
        ...
        self.codebooks.append(noCodeLayer(hidden_channels, embed_dim, nb_entries))

        self.decoders = nn.ModuleList([Decoder(embed_dim, hidden_channels, in_channels, res_channels, nb_res_layers, scaling_rates[0])])
        for i, sr in enumerate(scaling_rates[1:]):
            self.decoders.append(Decoder(embed_dim, hidden_channels, embed_dim, res_channels, nb_res_layers, sr))
    ...
    def forward(self, x):
        ...
        #print('done enc!')
        code_q, code_d, emb_id = self.codebooks[-1](encoder_outputs[-1])
        diffs.append((code_d**2).mean())
        #print('done codebook!')
        for dec in self.decoders[::-1]:
            if len(decoder_outputs):
                decoder_outputs.append(dec(decoder_outputs[-1]))
            else:
                decoder_outputs.append(dec(code_q))
        #print('done dec!')
        return decoder_outputs[-1], diffs, encoder_outputs, decoder_outputs, emb_id
    ...
    def training_step(self, batch, batch_idx):
        ...
        # autoencode
        aeloss, log_dict_ae = self.loss(qloss, x, xrec, 0, self.global_step, self.current_epoch+1,
        ...
        # discriminator
        discloss, log_dict_disc = self.loss(qloss, x, xrec, 1, self.global_step,self.current_epoch+1,
                                        last_layer=self.get_last_layer(), split="train")
        opt_d.zero_grad()
...
class VQVAE2_1(pl.LightningModule):
    def __init__(self,
            vq_modal: str                  = 'hmi_image',
            in_channels: int                = 1,
            hidden_channels: int            = 128,
    ...
    def forward(self, x):
        ...
        for enc in self.encoders:
            if len(encoder_outputs):
                ...
                encoder_outputs.append(enc(x))
        z = encoder_outputs[-1]
        diffs.append((z**2).mean())
        for dec in self.decoders[::-1]:
            if len(decoder_outputs):
                decoder_outputs.append(dec(decoder_outputs[-1]))
            else:
                decoder_outputs.append(dec(z))
    ...
    def decode_codes(self, *cs):
        ...
        for dec in self.decoders[::-1]:
            if len(decoder_outputs):
                ...
                decoder_outputs.append(dec(cs[-1]))
