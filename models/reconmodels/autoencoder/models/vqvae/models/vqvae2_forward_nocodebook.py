...
class CodeLayer(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor, code = True) -> Tuple[torch.FloatTensor, float, torch.LongTensor]:
        x = self.conv_in(x.float()).permute(0,2,3,1)
        if code == False:
            return x.permute(0,3,1,2), None, None
...
class VQVAE2(pl.LightningModule):
    ...
    def forward_nocodebook(self, x):
        ...
        for l in range(self.nb_levels-1, -1, -1):
            ...
            if len(decoder_outputs):
                nocode_q = torch.cat([encoder_outputs[l], decoder_outputs[-1]], axis=1)
            else:
                nocode_q = encoder_outputs[l]
            print(f'nocode_q shape: {nocode_q.shape}')

            nocode_outputs = [self.upscalers[i](c, upscale_counts[i]) for i, c in enumerate(nocode_outputs)]
            upscale_counts = [u+1 for u in upscale_counts]
            decoder_outputs.append(decoder(torch.cat([nocode_q, *nocode_outputs], axis=1)))

            nocode_outputs.append(nocode_q)
            upscale_counts.append(0)
    ...
    def _validation_step(self, batch, batch_idx, suffix=""):
        ...
        # logs, modals = self.log_images(batch)
    ...
    def log_images(self, batch, only_inputs=False, plot_ema=False, **kwargs):
        ...
        self.eval()
        print('1 begin forward nocodebook')
        with torch.no_grad():
            xrec = self.forward_nocodebook(x)
        # self.train()
        print('2 end forward nocodebook')
        if x.shape[1] > 3:
