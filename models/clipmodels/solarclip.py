...
class SolarCLIP(pl.LightningModule):
    ...
    def instantiate_basemodal_encode(self, base_modal_config):
        if self.base_model_key == 'hmi_image':
            ...
            raise NotImplementedError(f"Base model key {self.base_model_key} not supported, please choose from ['hmi_image']")
        
    def instantiate_pairedmodal_encode(self, paired_modal_config):
        model = instantiate_from_config(paired_modal_config)
        if model.__class__.__name__ == 'ViTMAE':
            self.encode_paired = model.encode
        else:
